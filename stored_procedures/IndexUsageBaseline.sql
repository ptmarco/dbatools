...
Date: November 2023
Updated by: Marco Assis
Date: May 2025 - Add Reset Procedure to clear index history and record timestamp
...
--Create [dbo].[IndexUsageStatsReset] stored procedure
CREATE OR ALTER PROCEDURE [dbo].[IndexUsageStatsReset]
AS
BEGIN
	...
	--Create [dbo].[IndexUsageStatsReset] table
	IF OBJECT_ID('[dbo].[IndexUsageStatsReset]') IS NULL
		CREATE TABLE [dbo].[IndexUsageStatsReset]
		(
			[reset] DATETIME NOT NULL
		);
	INSERT INTO [dbo].[IndexUsageStatsReset] ([reset])
	VALUES getdate();
	
END

--Create [dbo].[IndexUsageStatsMerge] stored procedure
CREATE OR ALTER PROCEDURE [dbo].[IndexUsageStatsMerge]
AS

BEGIN
	TRUNCATE TABLE [dbo].[IndexUsageStatsSnap]; --clean up the "snapshot" table 

	DECLARE @MyDatabase NVARCHAR(255) = 'dba_database' --replace with the database in which you created the above tables
